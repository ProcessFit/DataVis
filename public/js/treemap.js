...
var  x = d3.scaleLinear()
         ...
         x2 = d3.scaleLinear()
            .domain([0,partition_w])
            .range([0,partition_w]).clamp(true),
         x_ctx = d3.scaleLinear()
            .domain([0,partition_w])
            .range([0,partition_w]).clamp(true)
         y = d3.scaleLinear()
            .domain([0,partition_h])
            .range([0,partition_h]).clamp(true),
         y2 = d3.scaleLinear()
            .domain([0,partition_h])
            .range([0,partition_h]).clamp(true),
         y_ctx = d3.scaleLinear()
            .domain([0,partition_h])
            .range([0,partition_h]).clamp(true)

var c_depth = d3.scaleLinear() // Scaling for slider bar
       .range([2.0,-0.5]) // size
       .domain([0,20])
       .clamp(true);
...
// scales for Text Size Range
   var textSize = d3.scaleLinear() // Scaling for slider bar
         .range([.5, 1]) // size
         .domain([0,1])
         .clamp(true);
...
var partition = d3.partition()  // icicle / radial / treemap colours
            .size([partition_w, partition_h])
            .padding(0)
            .round(false);

var treemap = d3.treemap() // treemap
            .size([partition_w, partition_h])
            .round(false)
            .paddingInner(0);
...
var svg = d3.select("#svg_chart_container")
            .append("svg")
            .attr("id", "svg_chart")
...
context = chartDiv.append("div")
        .attr("id","svg_context_container")
        .attr("class","div_float")

var svg_ctxt = d3.select("#svg_context_container")
        .append("svg")
        .attr("id", "svg_context")
         // set size when re-scaling for window dimensions

// Elements resized during 're-scale' ... e.g. based on window dimensions
var context_group = svg_ctxt.append("g")
    .attr("class", "brush")
    // set size when re-scaling for window dimensions
...
var context_tagged = svg_ctxt.append("g")
...
var labels_group = svg.append('g')
      .attr("id","labels_group")


var navigation = d3.select("#nav_container")
                   .append("svg")
                   .attr("id","navigation")

var trail = navigation.append("g")
   .attr("id", "trail")
...
function addBrush(){
  ...
  context_group.call(brush)
                    .call(brush.move, lastS);
...
function reScale() {
  ...
  menu_height = d3.select("#navigationbar").node()
  ...
  svg_ctxt.style("width", context_pct*width+margin)
  ...
  trail.attr("transform","translate(0,"+(context_pct*height+padding/2-b.h/2)+")")
  d3.select("#nav_slider")
   .attr("transform","translate("+(context_pct*width+margin+padding)+",50)")
...
function make_treemap(theList, highlight = false){
   ...
   var planeMaterial = new THREE.MeshBasicMaterial(
                    {color: 0xffffff, vertexColors: THREE.FaceColors, opacity:1, transparent:true  })
   var materials = []
   ...
   theList.forEach(function(d,i){
               ...
               mesh_t.scale.set(d3.max([(d.x1 - d.x0),0.00000001]), d3.max([(d.y1 - d.y0),0.00000001]),1) // width
               for ( var j = 0; j < 2; j ++ ) {
                     face_id.push(d.id)
...
function make_thick_lines(theList, highlight = false){
    var planeGeometry = new THREE.PlaneGeometry(1, 1, 1);
    var planeMaterial = new THREE.MeshBasicMaterial(
                     {color: 0xffffff, vertexColors: THREE.FaceColors, opacity:0.5, transparent:true  })
    var materials = []
    var g = new THREE.Geometry();
...
function showNode(d,tolerance) {
   ...
   return ((d==current_zoom_node)|((d.depth >=(selected_level) &
...
function textXY(d){
         ...
         if ((y(partition_h-d.y1) < height) & (y(partition_h-d.y0) > 0)) {
             midpty = (d3.max([y(partition_h-d.y0),0]) + d3.min([height,y(partition_h-d.y1)]))/2
             midpty -= d3.min ([selected_level - d.depth, 3])*heighty*.05
         }
  ...
  labels.enter().append("text")
     ...
     .style("font-size", function(d) {
      ...
      .transition(d3.easeCubic).duration(0)
       .attr("transform", function(d) {return textPos(d)})
      .style("font-size", function(d) {
         if (d == current_zoom_node){ return '2.5rem'}
         var l = visOptions.labels_levels.value
         return  textSize((l-(d.depth-selected_level))/l) +'rem'
      })
   ...
   labels.exit().transition().
      duration(0).attr("opacity", 0).remove();
...
function label_text(d){
   ...
   if (d == current_zoom_node){ scale = 22}
...
function makeTween(){
   ...
   tween_current = {x0: x.domain()[0],
    ...
    tween.onUpdate(function(){
           zoomToCoord([tween_current.x0, tween_current.x1,
               tween_current.y0, tween_current.y1], false)
...
function updateGL(){
   ...
   three_x.range([partition_w/2*(1-zoom_x), partition_w/2*(1+zoom_x)])
   three_y.range([-partition_h/2*(1-zoom_x), -partition_h/2*(1+zoom_x)])
   mesh.scale.x = mesh_line2.scale.x = mesh_line.scale.x =  zoom_x
   mesh.scale.y = mesh_line2.scale.y = mesh_line.scale.y =  zoom_x
   mesh.position.x = mesh_line2.position.x = mesh_line.position.x = -three_x(x.domain()[0])
   mesh.position.y = mesh_line2.position.y = mesh_line.position.y = -three_y(y.domain()[0])
   if (mesh_highlight){
...
function fixAspect(coords, x_domain, y_domain){
   ...
   // calculate zoom_ [d.x0, d.x1,d.y0, d.y1] --> use minimum
      ...
      let x0 = coords[0], x1 = coords[1], y0 = coords[2], y1 = coords[3]
      zx = (x_domain[1]-x_domain[0])/(x1-x0)
      zy = (y_domain[1]-y_domain[0])/(y1-y0)
      padding = 1.08
      //console.log("Fix aspect", zx, zy)
      if (zx.toFixed(3) == zy.toFixed(3)) {return coords} // no correction required
      ratio = (x_domain[1]-x_domain[0])/(y_domain[1]-y_domain[0])
      if (zx < zy) { // keep x coords, adjust y
...
function zoomLevel() {
   ...
   if((d3.event.type =='wheel') & !visOptions.scrollwheel.value) {return}
...
function brush_started(){
  ...
  if ((brushStart[0][0] == brushStart[1][0]) & (brushStart[0][1] == brushStart[1][1])) {
     //context_group.call(brush.move,
   fixbrush([[0,0],[x_ctx.range()[1], y_ctx.range()[1]]])
   context_group.call(brush.move,
      [[0,0],[x_ctx.range()[1], y_ctx.range()[1]]]);
...
function zoomToCoord(coords,depth){
   ...
   moveTo = [[x_ctx(coords[0]), y_ctx(partition_h-coords[3])],
...
function zoomended(){
     ...
     if (((d3.event.sourceEvent.type =="mouseup") & (tol < 10)) |
...
function zoomRect(){
           ...
           rect_select_group
           .attr("transform","translate("+(x(d.x0)-2)+","+(y(partition_h-d.y1)-2)+")")
           .select("rect")
...
function withins() {
   ...
   within = thelist.filter((d)=> (d.x0>=x.domain()[0]) & (d.x1<=x.domain()[1]) & (d.y0>=(partition_h-y.domain()[1])) & (d.y1<=(partition_h-y.domain()[0])))
   console.log(within)
}
