...
var arc_context_tagged = context.append("g")
      .attr("id","arc_context_tagged")
...
function reScale() {
  ...
  let sb = d3.select("#sidebar").node().getBoundingClientRect()
       ...
       d3.select("#canvas_container2").style("left",(margin+(width1-width)/2 + (1-context_pct)*width/2)+"px").style("top","20px")
       ...
       arc_context_tagged.attr("transform",
                       "translate("+(width1/2+margin )+",20)")
...
function largestCurrent(){
 // nodes beyond range of chart
 return thelist.filter((d)=> (d.x0 >= rads.domain()[0]) & (d.x1 <= rads.domain()[1])).sort(compareByValue)[0  ]
}

function nodePartialReplace(){
 // left = 1, right = -1
 return thelist.filter(function(d) {
         return ((rads(d.x1) - rads(d.x0))> (Math.PI/8)) |
...
function viewArc2(d){
    arc.innerRadius((d)=> d==root? 0 : x(context_pct*0.8*(d.depth+1)*barheight))
       .outerRadius((d)=> x(context_pct*0.8*(d.depth+2)*barheight))
       .startAngle((d)=> -rads2(d.x0)+Math.PI)
       .endAngle((d)=> -rads2(d.x1)+Math.PI)
   return arc(d)
}
...
function tagNodes_arcs(triggeredByTag) {
  ...
  tagBars = arc_select_tagged.selectAll("path")
      .data(tagged_, function(d) {return d.id})
  tagBars.enter().append("path")
   ...
   if(!triggeredByTag) return

   //drawContextArc()
   var tagged_helpers = tagged.concat(helper_check)
   tagBars2 = arc_context_tagged.selectAll("circle")
       .data(tagged_helpers)
   tagBars2.enter().append("circle")
